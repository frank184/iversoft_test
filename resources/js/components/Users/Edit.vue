<template>
  <div>
    ...
    <form @submit="update" :action="`http://localhost:8000/api/users/${user.id}`" class="w-full max-w-md">
      ...
      <div class="flex justify-between items-center">
        <input type="submit" name="commit" value="Update User" class="bg-blue hover:bg-blue-dark text-white font-normal py-2 px-4 rounded">
        <hr>
...
<script>
    export default {
      ...
      created() {
        axios.get(`http://localhost:8000/api/users/${this.$props.id}`)
        .then(res => res.data)
